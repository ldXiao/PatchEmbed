...
void WeightedGlobalLocal::solve_weighted_arap(const Eigen::MatrixXd& V, const Eigen::MatrixXi& F,
  ...
  Eigen::VectorXd Uc;
  if (dim == 2) {
    if(!m_state.is_hard_const){
      SimplicialLDLT<SparseMatrix<double> > solver;
      Uc = solver.compute(L).solve(rhs);
    }else{
      rhs = rhs*(-1);
      VectorXd bc=VectorXd::Zero(soft_bc_p.rows()*dim);
      VectorXi b=VectorXi::Zero(soft_bc_p.rows()*dim);
      //VectorXd bc=VectorXd::Zero(20);
      //VectorXi b =VectorXi::Zero(20);
      SparseMatrix<double> Aeq(b.rows(),dim*V.rows());
      b<<soft_b_p,soft_b_p.array()+V.rows();
      bc<<soft_bc_p.col(0),soft_bc_p.col(1);

    //   for(int r=0;r<b.rows();r++){
    //     Aeq.insert(b(r),b(r))=1;
    //   }
    //   //cout<<b<<endl;
    //   Aeq.makeCompressed();
      VectorXd Beq = VectorXd::Zero(b.rows());
    //   for(int r=0;r<b.rows();r++){
    //       Beq(r)=bc(r);
    //   }
      igl::min_quad_with_fixed_data<double> data;
      igl::min_quad_with_fixed_precompute(L,b,Aeq,true,data);
      igl::min_quad_with_fixed_solve(data,rhs,bc,Beq,Uc);
    }
...
double WeightedGlobalLocal::compute_energy(Eigen::MatrixXd& V_new) {
  ...
  if(!m_state.is_hard_const)
    ...
    return compute_energy_with_jacobians(m_state.V,m_state.F, Ji, V_new,m_state.M);
  }
}
...
/// Slim Implementation
  ...
  data.is_hard_const = is_hard_const;
