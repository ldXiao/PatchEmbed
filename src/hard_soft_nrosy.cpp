...
std::vector<Eigen::Triplet<std::complex<double>>> to_triplets(Eigen::SparseMatrix<std::complex<double>> & M){
    std::vector<Eigen::Triplet<std::complex<double>>> v;
    for(int i = 0; i < M.outerSize(); i++)
        for(typename Eigen::SparseMatrix<std::complex<double>>::InnerIterator it(M,i); it; ++it)
...
MatrixXd Hard_nrosy
        (
                ...
                const MatrixXi& F,          // Faces
                const MatrixXi& TT,         // Adjacency triangle-triangle
                const VectorXi& hard_id,    // Hard constraints face ids
                const MatrixXd& hard_value, // Hard constraints 3d vectors
                const int n                 // Degree of the n-rosy field
        )
{
    ...
    // Build the sparse matrix, with an energy term for each edge
    std::vector< Triplet<std::complex<double> > > t;
    std::vector< Triplet<std::complex<double> > > l;
    std::vector< Triplet<std::complex<double> > > tb;
    ...
    for (unsigned f=0;f<F.rows();++f)
    {
        for (unsigned ei=0;ei<F.cols();++ei)
        {
            ...
            // Add the term conj(f)^n*ui - conj(g)^n*uj to the energy matrix
            l.push_back(Triplet<std::complex<double> >(count,f,    std::pow(std::conj(ef),n)));
            l.push_back(Triplet<std::complex<double> >(count,g,-1.*std::pow(std::conj(eg),n)));
//            cout << t.size() << endl;
            ++count;
        }
    }
    ...
    L.setFromTriplets(l.begin(), l.end());
    SparseMatrixXcd LL = L.adjoint() * L;
    for(int i = 0; i < L.outerSize(); i++)
        for(typename Eigen::SparseMatrix<std::complex<double>>::InnerIterator it(LL,i); it; ++it)
            t.push_back(Triplet<std::complex<double> >(it.row(),it.col(), std::complex<double>(2,0) * it.value()));
    count = F.rows();
    ...
    for (unsigned r=0; r<hard_id.size(); ++r)
    {
        ...
        std::complex<double> c(v.dot(T1.row(f)),v.dot(T2.row(f)));
        t.push_back(Triplet<std::complex<double> >(count,f, std::complex<double>(1.0,0)));
        t.push_back(Triplet<std::complex<double> >(f,count, std::complex<double>(1.0,0)));
        tb.push_back(Triplet<std::complex<double> >(count,0, std::pow(c,n) * std::complex<double>(1.0,0)));
        ++count;
//        cout << count << endl;
    }
    // Solve the linear system
    typedef SparseMatrix<std::complex<double>> SparseMatrixXcd;
    SparseMatrixXcd A(count, count);
...
//    cout  << A.rows() << A.cols() <<endl;
    ...
    solver.compute(A);
    assert(solver.info()==Success);
    MatrixXcd u = solver.solve(MatrixXcd(b));
    assert(solver.info()==Success);
